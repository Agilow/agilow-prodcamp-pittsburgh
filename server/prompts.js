...
=== AGILOW ICP ===
Early-stage ROBOTICS company that builds and/or deploys physical robots / autonomous physical systems and is deploying its FIRST robots (first ~10 units / first real field deployments, not scaled or mature); a SMALL/early team — typically pre-seed/seed; 2-10 people is ideal but anything up to ~50 (11-50 is fine) still fits; raised seed or Series A within ~18 months; led by a technical founder/CTO. Physical deployment domains such as warehouse/logistics, transportation/AV, agriculture, manufacturing/industrial, construction, or AVIATION/AEROSPACE robotics. Computer vision is NOT required — a robotics company that does not use computer vision still fits. Already having a PM does NOT disqualify them — it is neutral-to-positive, never a negative.
305 | === RULES ===
...
319 |   - AVIATION and AEROSPACE robotics ARE in domain (DOMAIN: YES) — e.g. drones, eVTOL, autonomous aircraft, space/aerospace robotics deploying their first units all count. Do NOT exclude or downgrade them for being aviation/aerospace.
...
323 | A. SMALL/early team? TRUE for any small/early team up to ~50 people — a tiny 2-10 person robotics startup deploying its first pilot is IDEAL, and 11-50 is also fine, all TRUE (small is good; there is NO lower floor — never fail A for being small). FALSE only when clearly TOO BIG: roughly ~100+ people, or a mature/established org operating at scale with a fully built-out delivery/PM system. MID-SIZE 50-100 people is a middle case: A is still TRUE but the rating is CAPPED at Moderate (see scoring). "2-10" => TRUE. "11-50" => TRUE. "50-100" => TRUE but caps at Moderate. "~100+" => FALSE.
...
SCORING — apply MECHANICALLY. The written verdict MUST match these rules exactly. Do NOT invent any disqualifier beyond the domain and A/B/E rules below (aviation/aerospace robotics is IN domain; CV and PM presence are NOT disqualifiers):
- A criterion resting on a [LOW-CONFIDENCE] or [UNVERIFIED] fact counts as "unknown" and cannot support "Strong".
- WEAK if: DOMAIN is NO (not robotics / pure software with no physical robot) OR A is false (team clearly TOO BIG / large-and-established at scale: roughly ~100+ people, or a mature org operating at scale with a built-out delivery/PM system) OR B is false (no raise in ~the last 24 months) OR E is false (clearly large/old/established-at-scale). Otherwise, missing/unknown data NEVER makes a lead Weak. NEVER fail A — or rate a lead Weak/Moderate — for being SMALL (2-10 is IDEAL) or for being 11-50; and NEVER downgrade for being aviation/aerospace, for lacking computer vision, or for already having a PM.
...
- MID-SIZE HEADCOUNT (roughly 50-100 people) CAPS the rating at Moderate (A stays true, not Weak). 100+ is too big => A false => Weak. Up to ~50 has NO cap.
- STRONG = DOMAIN yes (real robotics, including aviation/aerospace) AND A true (small/early team up to ~50) AND B true AND E true (all on VERIFIED facts) AND C true (in the first-deployment band / building toward first pilots). CV is irrelevant to the verdict; aviation/aerospace is in domain; D (PM presence) is irrelevant and may be true, false, or unknown.
- MODERATE = not Weak and not Strong (e.g. C false, mid-size 50-100 headcount, or exactly one of A/B/E is unknown while none is false). CV-BASED no/unknown does NOT make a lead Moderate.
- CONSISTENCY CHECK before writing the verdict: if DOMAIN is YES and none of A/B/E is false and none is unknown, the lead is at minimum Moderate (it CANNOT be Weak). If DOMAIN is YES AND A/B/E are all true on VERIFIED facts AND C is true, it IS Strong (CV, aviation/aerospace, and PM presence are all irrelevant to the verdict).
...
363 | ICP FIT CHECKS:
364 | 0. DOMAIN (robotics company that builds/deploys physical robots? aviation/aerospace robotics counts as YES; pure software with no physical robot = no): <yes|no> — <evidence>
364b| CV-BASED (relies on computer vision / perception? — NEUTRAL descriptive note, ZERO effect on the verdict): <yes|no|unknown> — <evidence>
365 | A. Small/early team (up to ~50 = true, no lower floor; 50-100 = true but caps at Moderate; ~100+ = false/too big): <true|false|unknown> — <evidence>
